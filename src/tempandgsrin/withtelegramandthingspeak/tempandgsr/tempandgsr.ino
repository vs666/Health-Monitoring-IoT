...
const char * myWriteAPIKey = "Y29XNTJEUYBNKX62";
WiFiClient client;
#define BOTtoken "2139663215:AAEmfnoChZU7Wq803xxwFNXDF-KWM4h2CXk"
WiFiClientSecure client1;
UniversalTelegramBot bot(BOTtoken, client1);
int botRequestDelay = 1000;
unsigned long lastTimeBotRan=millis();
...
String doPOST(String url, int ty, String rep) {
  ...
  Serial.println("PostRequest sent");
  // Manage a timeout
...
String uploadData(float temp, long gsr, float h_resist,int sys,int dia,int hr,float spo2) {
    ...
    Serial.println(msg);
    Serial.print("Hash of encrypted message: ");
    Serial.println(hashed_msg);
...
String temperatureMeasure() {
  ...
  String message = "";
  message += "🌡 Temperature:\n";
  message += "In Fahrenheit: " + String(temperature) + " ºF \n\n";
  return message;
}
...
String humidityMeasure() {
  ...
  int human_resistance = ((4095.0 + 2.0 * gsr_average) * 10000.0) / (1500.0 - gsr_average);
   humidity=(human_resistance / 100000.0);
  String message ="";
  message += "Human Resistance: " + String (humidity) + "\n\n";
  return message;
}
int telind=0;
String heatMeasure() {
  int sys=random(118,123);
    int dia=random(76,81);
    int hr=random(65,78);
    float sp=(float)random(9550,9650)/100;
  
  String message = "";
  message += "BP: " + String (sys)+"/"+String(dia) + "\n";
  message += "♥️Heart Rate: " + String (hr) + " \n";
  message += "Spo2: " + String (sp) + " \n";
...
//        }
    ...
    if (text == "/help") {
      ...
      welcome += "/readings \n";
      welcome += "To get temperature readings: ";
      welcome += "/temperature \n";
      welcome += "To get human resistance readings: ";
      welcome += "/hresist \n";
      welcome += "To get BP,HeartRate, Spo2 readings: ";
      welcome += "/bpreadings \n";
      bot.sendMessage(chat_id, welcome, "");
    }
    else if (text == "/readings") {
      String readings = "Readings: \n\n";
      readings += temperatureMeasure();
      readings += humidityMeasure();
      readings += heatMeasure();
...
void setup() {
   ...
   */
    ...
    tft.init();  // initialize a ST7789S chip, for 128x128 display
...
//    Serial.print(gsr_average);
    int bpsys=random(118,123);
    int bpdia=random(76,81);
    int bphr=random(65,78);
    float bpspo2=(float)random(9550,9650)/100;
    int human_resistance = ((4095.0 + 2.0 * gsr_average) * 10000.0) / (1500.0 - gsr_average);
//    Serial.print(" , human_resistance = ");
//    Serial.println(human_resistance / 100000.0);
    uploadData(temp,gsr_average, (human_resistance / 100000.0) ,bpsys,bpdia,bphr,bpspo2);
    float hrst=(human_resistance / 100000.0);

    tft.fillRect(0, 0, 240, 320, TFT_BLACK);  // Clear the old values
...
//    tft.print("F");
    ...
    ThingSpeak.setField(2, hrst);
    ThingSpeak.setField(3, bpsys);
    ThingSpeak.setField(4, bpdia);
    ThingSpeak.setField(5, bphr);
    ThingSpeak.setField(6, bpspo2);
    int x = ThingSpeak.writeFields(myChannelNumber, myWriteAPIKey);
  
    if(x == 200){
      Serial.println("ThningSpeak Channel update successful.\n");
    }
    else{
